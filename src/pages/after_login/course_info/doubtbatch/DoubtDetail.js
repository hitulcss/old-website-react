...
export default function CommunityDetail({
      ...
      if (!isEditComment?.show) {
        if (replyComment == -1) {
          createBatchDoubtComment(
            {
              msg: msg,
              batchDoubtId: selectedCommunity,
              batchId: batchId,
            },
            file
          );
        ...
        if (isEditComment?.replyComment) {
          editReplyComment({
            ...
            msg,
            batchCommunityId: particularBatchDoubtData?.data?.id,
          });
  ...
  const handleLikeDislike = (id) => {
    batchDoubtLikeAndDislike({ batchDoubtId: id }, "");
  };
  ...
  useEffect(() => {
    // console.log('me callled', particularBatchDoubtData)
    if (batchCommunityCommentsData) {
      if (replies?.show) {
        batchCommunityCommentsData?.data?.comments?.map((item) => {
          ...
          <div className="doubt-detail-parent">
            <div className="post-details">
              <div className="post-details-left">
                <div className="post-details-left-user-profile">
                  <Avatar
                    src={particularBatchDoubtData?.data?.user?.profilePhoto}
                  />
                </div>
                <div className="post-details-left-batch-detail">
                  <div className="post-details-left-batch-name">
                    {particularBatchDoubtData?.data?.user?.name}

                    {!particularBatchDoubtData?.data?.isVerified && (
                      <RiVerifiedBadgeFill
                  ...
                  <div className="post-details-left-user-name">
                    {particularBatchDoubtData?.data?.createdAt}
                  </div>
                </div>
              </div>
            ...
            <div className="post-content-middle">
              ...
              <div className="post-title detail-post-title">
                <p style={{ color: "var(--primaryColor)" }}>
                  {particularBatchDoubtData?.data?.lectureName} |{" "}
                  {particularBatchDoubtData?.data?.subject}
                </p>
                {particularBatchDoubtData?.data?.desc !== "" && (
                  <p style={{ marginTop: "10px" }}>
                    {particularBatchDoubtData?.data?.desc}
                  </p>
                )}
              </div>
              {particularBatchDoubtData?.data?.problemImage !== "" && (
                <div
                  className="post-content"
                  onClick={() => setDoubtOpen(true)}
                >
                  <img
                    src={particularBatchDoubtData?.data?.problemImage}
                    onClick={(e) => {
                      e.stopPropagation();
                      setImgopen(true);
                      setImgsrc(particularBatchDoubtData?.data?.problemImage);
                    }}
                  />
                </div>
              )}
            ...
            <div className="post-comments">
              <div className="doubt-like">
                {particularBatchDoubtData?.data?.isLiked ? (
                  <IoIosHeart
                    className="doubt_icon heart"
                    onClick={() =>
                      handleLikeDislike(particularBatchDoubtData?.data?.id)
                    }
                  />
                ) : (
                  <IoIosHeartEmpty
                ...
                {particularBatchDoubtData?.data?.likes}
              </div>
              <div className="doubt-comment">
                <FaRegComment className="doubt_icon" />
                {particularBatchDoubtData?.data?.comments?.length}
                {/* {particularBatchDoubtData?.data?.} */}
              </div>

              {particularBatchDoubtData?.data?.isResolved ? (
            ...
            <div className="doubt-detail-lower">
              {true && particularBatchDoubtData?.data && (
                <div className="doubt-detail-cmnts main-comments">
                  ...
                  {particularBatchDoubtData?.data?.comments?.length > 0 ? (
                    particularBatchDoubtData?.data?.comments?.map(
                      (item, index) => {
                        return (
                          <div
                            key={index}
