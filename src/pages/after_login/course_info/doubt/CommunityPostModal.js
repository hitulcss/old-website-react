...
const CommunityPostModal = ({
  ...
  const { batchCommunityLikeAndDislike, deleteCommunity } =
    useContext(CoursesData);

  const handleLikeDislike = async (id) => {
    if (!prePurchase) {
      batchCommunityLikeAndDislike({ batchCommunityId: id }, batchId);
    } else {
      toast.dismiss();
      setLockModal(true);
  ...
  return (
    <div className="doubt-post-parent">
            ...
            {data?.isMyCommunity && (
              <Typography
                onClick={() => {
                  ...
                  // deleteCommunity({ batchCommunityId: data?.id, batchId })
                }}
                sx={{
                  ...popOverItemStyle,
                  fontSize: "1rem",
      ...
      >
        {isExpanded ? data?.desc : `${data?.desc.slice(0, 200)}...`}
