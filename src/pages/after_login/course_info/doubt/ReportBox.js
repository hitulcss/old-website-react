...
const ReportBox = ({ setShowReport, showReport }) => {
  //context
  const { reportCommunity, reportComment, reportReplyComment } =
  ...
  const handleReport = () => {
    if (reason?.radioField !== -1) {
      ...
      if (showReport?.type == "post") {
        reportCommunity({ id: showReport?.data?.id, reason: reasonText });
        setReason({ radioField: -1, textField: "" });
      }
      if (showReport?.type == "comment") {
        reportComment({ id: showReport?.data?.commentId, reason: reasonText });
        setReason({ radioField: -1, textField: "" });
      }
