...
const PaidCourses = ({ from }) => {
  ...
  let paidMycourses = myCourses?.filter((item) => item?.isPaid);
      ...
      <div className="after-login-mycourse-wrapper">
        <div className="new_courses_container">
          <Wrapper>
            {!loadingForMyCourse ? (
              from == "after-login" && (
                <div className="mycourse_container">
                  {paidMycourses?.length > 0 ? (
                    paidMycourses?.map((item, index) => (
            ...
            {/* <Course_Exam_Box
              courses={from == 'after-login' ? paidMycourses : myCourses}
              from={from}
            // courses={
            //   filter == "all"
